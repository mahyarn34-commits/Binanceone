...
BOT_CODE_VERSION = "v3-diag-4h300"  # برای تأیید اینکه دیپلوی جدید واقعاً فعاله


def run_market_scan() -> str:
    """اسکن بازار رو انجام می‌ده و متن نهایی پیام تلگرام رو برمی‌گردونه"""
    log.info("Starting market scan (top %s symbols by volume)... [code_version=%s]", config.TOP_N_SYMBOLS, BOT_CODE_VERSION)
    ...
    for symbol in symbols:
        try:
            ...
            if not result["aligned"]:
                if result["trend_4h"].trend not in ("BULLISH", "BEARISH"):
                    fail_4h_neutral += 1
                else:
                    fail_1h_not_confirmed += 1

            # یک نمونه‌ی کامل رو با جزئیات تو لاگ می‌ندازیم تا دقیقاً معلوم بشه
            # قطع‌شدن کجای زنجیره (4H/1H/M15) اتفاق افتاده.
            if not sample_logged:
                sample_logged = True
                log.info(
                    "Sample diagnostic [%s]: 4h_trend=%s 1h_trend=%s aligned=%s bias=%s m15_score=%s/100",
                    symbol, result["trend_4h"].trend, result["trend_1h"].trend,
                    result["aligned"], result["bias"], m15.score,
                )
    ...
    log.info(
        ...
        "4h_neutral_fails=%s 1h_not_confirmed_fails=%s [code_version=%s]",
        best_symbol, best_score, config.SIGNAL_SCORE_THRESHOLD,
        fail_4h_neutral, fail_1h_not_confirmed, BOT_CODE_VERSION,
    )
    ...
    return format_report(
        ...
        fail_4h_neutral=fail_4h_neutral,
        fail_1h_not_confirmed=fail_1h_not_confirmed,
    )
...
def format_report(
    ...
    errors, best_symbol=None, best_score=None, fail_4h_neutral=0, fail_1h_not_confirmed=0,
) -> str:
    ...
    if not buy_signals and not sell_signals:
        ...
        lines.append(
            f"🔍 تشخیص: {fail_4h_neutral} نماد در فیلتر 4H متوقف شدن (روند NEUTRAL) | "
            f"{fail_1h_not_confirmed} نماد در تأیید 1H رد شدن"
        )
        lines.append("")
